...
export default function JeSuisConducteurAfficherTousMesCovoituragesAvecUnPassager(){
    ...
    const { passagerId, conducteurId, prenom } = useParams();
    ...
    useEffect(()=>{
        async function fetchData(){
            const result = await Requete.get(
                "/mesCovoiturages/conducteur/un-passager/" + passagerId + "/" + conducteurId,
    ...
    const submit = async function(e){
        ...
        try{
            ...
            await Requete.post(
                "/mesCovoiturages/conducteur/sortir-un-passager/" + passagerId + "/" + conducteurId,
                sortirPassager,
                { headers: { "x-auth-token": token } }
            );
            history.push("/mesCovoiturages/" + conducteurId);
        } catch(err){
                ...
                <div className="container">
                    <form onSubmit={submit}>
                        ...
                        <div className='form-group'>
                            ...
                            <Link to={"/mesCovoiturages/passager/profil-partiel/" + passagerId + "/" + conducteurId}>Page précédente</Link>
                        </div>
                    </form>
                </div>
            )}
