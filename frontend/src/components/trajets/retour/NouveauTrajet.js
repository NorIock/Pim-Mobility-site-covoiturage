...
export default function CreerNouveauRetour(){

    const [prenom, setPrenom] = useState(); // Pour récupérer le prénom du membre
    ...
    // let optionItemsDepart = depart_data.map((departs => 
    //     <option key={departs.nom} value={departs.nom}>{departs.nom}</option>
    // ));

    // let optionItemsDestination = destination_data.map((destination => 
    //     <option key={destination.nom} value={destination.nom}>{destination.nom}</option>
    // ));

    // let heuresOptionItems = heuresData.map((heures => 
    //     <option key={heures.chiffre} value={heures.chiffre}>{heures.chiffre}</option>
    // ));

    // let minutesOptionItems = minutesData.map((minutes => 
    //     <option key={minutes.chiffre} value={minutes.chiffre}>{minutes.chiffre}</option>
    // ));
    ...
    const submit = async function(e){
        ...
        try{
            const newRetour = { checkItems,prenom, id, depart, quartierDepartData, heures_de_depart, minutes_de_depart, destination, quartierDestinationData, nombre_de_places, nouveauTrajet };
            await Requete.post(
                "/trajetsRetour/creer/" + id,
                newRetour,
                { headers : { "x-auth-token": token}},
            );
    ...
    return(
        <div className="col-md-12">
            <div className="card card-container">
                ...
                <div className="card-body inside-card-body">
                    <h4>Ajouter un ou plusieurs trajets retour:</h4>
                </div>
                <form onSubmit={submit}>
                    ...
                    <div className="form-group">
                        ...
                        >
                            <option>Choisissez votre zone de départ</option>
                            {optionItemsDepart}
                        </select> */}
                    </div>
                    ...
                    <div className='form-group'>
                        ...
                        >
                            <option>Choisissez votre zone d'arrivée</option>
                            {optionItemsDestination}
                        </select> */}
                    </div>
