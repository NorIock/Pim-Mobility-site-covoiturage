...
export default function AfficherDetailTrajetsRetourDestination(){
    ...
    useEffect(()=>{
        ...
        async function fetchData(){
            const result = await Requete.get(
                "/trajetsRetour/destination/tous/" + nom,
        ...
        <div>
            {data.length === 0 ? (
                ...
                <div className="container" style={{ border: "3px solid #317681", textAlign: "center", marginTop: "80px"}}>
                    <h3>Aucuns trajets retour à destination de {nom}</h3>
                    <br></br>
                ...
                <div className="container" style={{marginTop: "80px"}}>
                    <table className="table affichage-profil">
                        <thead>
                            <tr>
                                <th style={{ textAlign: "center"}}><h5>Trajets retour à destination de {nom}: {data.length}</h5></th>
                            </tr>
                        </thead>
                        <tbody>
                            {data.map(retour => (
