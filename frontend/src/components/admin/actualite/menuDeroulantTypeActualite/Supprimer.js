...
export default function SupprimerTypeActualite(){
    ...
    const supprimer = async function(){
        ...
        try{
            await Requete.delete(
                "/actualites/menu-deroulant/supprimer/" + id,
    ...
    return(
        ...
        {data === null ? (
            ...
            <div>
                ...
                <h4>Souhaitez-vous supprimer ce type d'actualité: <strong>{data.nom}</strong> ?</h4>
                <button onClick={() => supprimer()} className="btn btn-danger">Supprimer</button>
                <p><Link to="/admin/type-actualite/gerer">Retour</Link></p>
            </div>
        )}
        </div>
    )
