...
export default function AfficherNotificationsVues(){

    const [data, setData] = useState([]);
    const [origine] = useState("afficher-vues")
    ...
    useEffect(()=>{
        async function fetchData(){
            const result = await Requete.get(
                "/notifications/admin/afficher-vues",
            ...
            {data.length === 0 ? (
                ...
                <div>
                    ...
                    <div style={{textAlign: "center"}}>
                        <h3>Notifications vues mais pas ouvertes:</h3>
                            ...
                            <ul className="list-group list-group-flush" key={notifications._id}>
                                <li className="list-group-item">
                                    <div style={{display: "flex", alignItems: "center", justifyContent: "space-between"}}>
                                        <h4 style={{textDecoration: "underline"}}>Vue le: {new Intl.DateTimeFormat('fr-Fr',{ month:'numeric',day:'2-digit',year:'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric'}).format(new Date(notifications.date_vue))}</h4>
