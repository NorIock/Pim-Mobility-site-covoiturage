...
export default function MatchedProfilAller(){
    ...
    const [demandeEnvoyee, setDemandeEnvoyee] = useState(false); // Permet d'afficher un message et une redirection vers les trajets retour du même membre quand la demande est passée
    ...
    const [allerOuRetour] = useState("aller"); // Comme il s'agit du matching aller, la demande dans la bdd sera indiqué comme étant pour un trajet aller
    const { id, membreId } = useParams(); // id est l'id de la personne qui reçoit la demande de covoiturage, membreId est l'id de la personne qui demande à faire parti du covoiturage
    ...
    const submit = async function(e){
        ...
        try{
            ...
            await Requete.post(
                // membreId est l'id de la personne qui demande à faire parti du covoiturage
                ...
                <div className="card" style={{width:"60rem"}}>
                    ...
                    <form onSubmit={submit}>
                            ...
                            <div>
                                ...
                                <h5><label className="my-1 mr-2">Souhaitez vous faire cette demande en tant que:</label></h5>
                                    <select
                                        ...
                                        // onChange={(e) => setPassagerOuConducteur(e.target.value)} 
                        ...
                        <table className="table">
                            ...
                            <tbody>
                                {trajetsData.map(aller => (
                                    <tr key={aller._id}>
                                        <td>
                                            <label style={{textAlign:"center"}}>
                                                <CheckBox
                                                    name={aller._id}
                                                    checked={checkItems[aller._id]}
                                                    onChange={HandleChange}
                                                />
                                            </label>
                                        </td>
                                        <td>
                                            <p><strong>{aller.jour}:</strong> de {aller.depart}{aller.depart_quartier && <span>, {aller.depart_quartier}</span>} à {aller.heure_de_depart_en_string} pour {aller.destination}{aller.destination_quartier && <span>, {aller.destination_quartier}</span>} {passagerOuConducteur !== "Conducteur" && <span>Places disponibles: {aller.nombre_de_places}</span>}</p>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                        ...
                        {demandeEnvoyee && 
                            <div>
                                ...
                                <h5 style={{textAlign: "center"}}><Link to={"/matching/retour/profil/" + id + "/" + membreId}>Voir les trajets retours de {data[0].prenom}</Link></h5>
                            </div>
                        }
                        <div className='form-group'>
                            <input type='submit' value='Faire une demande de covoiturage' className='btn btn-primary float-right'/>
                            <Link to={"/matching/aller/" + membreId}>Page précédente</Link>
                        </div>
                    </form>
                </div>
            }
